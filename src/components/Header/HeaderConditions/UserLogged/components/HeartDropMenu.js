...
const HeartDropMenu = ({ dropMenuHeart, setDropMenuHeart }) => {
    ...
    useEffect(() => {
        ...
        const getActivities = async () => {
            ...
            userData.posts.forEach(elem => {
                ...
                const postComments = elem.comments.filter(elemComment => elemComment.userUid !== userRedux.uid)
                    .map(elemComment => ({ ...elemComment, type: 'comment', likes: { ...elemComment.likes }, user: { ...elem.user }, post: { uid: elem.uid, imageUrl: elem.image } }))
    ...
    return (
        <DropMenu
            ...
            styleForContainerBlock={`absolute w-[22rem] sm:w-[32rem] h-[18rem] shadow-defaultModal rounded bg-white flex items-center top-10 -right-10 p-0 m-0 z-30 cursor-pointer`}
                ...
                allActivities.length > 0 ?
                    ...
                    (
                        <div className="flex items-center justify-center flex-col h-full">
                            ...
                            <p className="text-sm max-w-[420px] text-center mt-3 px-3">When someone likes or comments on one of your posts, you'll see it here.</p>
                        </div>
                    )
            }
