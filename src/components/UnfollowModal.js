...
const UnfollowModal = ({ imageUrl, displayName, activeModal, setActiveModal }) => {

    const currentProfileUserRedux = useSelector((state) => state.currentProfileUserReducer.user)
    const hendleMenu = () => {
        setActiveModal(false)
    }
    const {name, uid} = currentProfileUserRedux
    const { hendleFollow } = useFollow({name, uid, displayName: currentProfileUserRedux.displayName, imageUrl: currentProfileUserRedux.imageUrl})
        ...
        <div onClick={(e) => e.stopPropagation()} className={`bg-white w-[25rem] rounded-xl duration-300 ${activeModal ? 'scale-100' : 'scale-50'}`}>
            <div className="flex flex-col justify-center items-center">
                ...
                <div className="w-full border-t h-12 flex itens-center justify-center active:bg-gray-100">
                    <button onClick={hendleFollow} className="text-sm text-red-500 font-bold w-full">Unfollow</button>
                </div>
